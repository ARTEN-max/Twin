...
async function apiFetch<T>(
  ...
  if (!response.ok) {
    throw new ApiError(data.message || 'An error occurred', response.status, data.error);
  }
...
export async function getRecordingJobs(userId: string, recordingId: string): Promise<Job[]> {
...
export async function retryDebrief(userId: string, recordingId: string): Promise<void> {
  await apiFetch(`/api/recordings/${recordingId}/retry-debrief`, { method: 'POST' }, userId);
}

export async function retryTranscription(userId: string, recordingId: string): Promise<void> {
  await apiFetch(`/api/recordings/${recordingId}/retry-transcription`, { method: 'POST' }, userId);
}
...
export async function getDownloadUrl(
  ...
  }>(`/api/recordings/${recordingId}/download-url`, { method: 'GET' }, userId);
  return response.data;
}
...
export async function getChatSession(
  ...
  if (!res.ok) {
    ...
    throw new ApiError(data.message || 'Failed to load chat session', res.status, data.error);
  }
  return res.json();
}
...
export async function generateChatOpener(
  userId: string,
  date?: string
): Promise<{ alreadyHasOpener: boolean; message: ChatMessage }> {
  const res = await fetch(`${API_BASE}/api/chat/opener`, {
    method: 'POST',
    headers: {
  ...
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
    throw new ApiError(data.message || 'Failed to generate opener', res.status, data.error);
  }
  return res.json();
}
